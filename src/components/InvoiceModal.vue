...
export default {
  ...
  methods: {
    ...
    deleteInvoiceItem(id) {
      // filter out the one with different ID
    ...
    async updateInvoice() {
      ...
      // make a PUT Request to update the firestore data
      try {
        const result = await fetch(
          `https://vue-invoice-app-6f5f2-default-rtdb.europe-west1.firebasedatabase.app/invoices/${this.id}.json`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
